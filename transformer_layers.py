...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        Inputs:
         ...
         ⎪sin(𝑖⋅10000−𝑗𝑑)cos(𝑖⋅10000−(𝑗−1)𝑑)if j is evenotherwise 
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        value = value.view(N, T, self.n_head, E//self.n_head).transpose(1, 2)#N,H,T,E/H
        key = key.view(N, T, self.n_head, E // self.n_head)
        key = key.permute(0,2,1,3) # N,H,T,E/H
        query = query.view(N, S, self.n_head, E // self.n_head)
        query = query.permute(0,2,1,3) # N,H,S,E/H
        var = query @ key.mT #N,H,S,T
        var  = var* (self.head_dim **-0.5)
        if attn_mask is not None:
            var= var.masked_fill(attn_mask==0, float('-inf'))
        prob = F.softmax(var, dim=-1)#N,H,S,T
        prob = self.attn_drop(prob)
        output = prob@value#N,H,S,E/H
        output = output.permute(0,2,1,3).contiguous().view(N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        '''
        patches_flat = F.unfold(x, kernel_size=self.patch_size, stride=self.patch_size)
        patches = patches_flat.transpose(1, 2)
