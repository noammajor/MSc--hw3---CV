...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        ####################################################################
        mean = extract(self.sqrt_alphas_cumprod, t,x_t.shape)
        std = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        x_start = (x_t -std*noise)/mean
        ####################################################################
        return x_start
    ...
    def q_sample(self, x_start, t, noise):
        ...
        ####################################################################
        mean = extract(self.sqrt_alphas_cumprod, t, x_start.shape)
        std = extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
        x_t =mean*x_start +std * noise
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        val = self.model(x_t, t, model_kwargs=model_kwargs)
        loss=(loss_weight*(val - target) ** 2).mean()
