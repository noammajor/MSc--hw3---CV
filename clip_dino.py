...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    ############################################################################
    txt_t = clip.tokenize(class_texts)
    txt_t=txt_t.to(device)
    new_im=[]
    for image in images:
        temp_image = Image.fromarray(image)
        new_im.append(clip_preprocess(temp_image))
    im= torch.stack(new_im).to(device)
    output_txt = clip_model.encode_text(txt_t)
    output_im = clip_model.encode_image(im)
    sim=get_similarity_no_loop(output_im, output_txt)
    indx= torch.argmax(sim, dim=1)
    pred_classes = [class_texts[idx] for idx in indx]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        for image in images:
            ...
            new_im.append(clip_preprocess(image))
        self.images= torch.stack(new_im).to(self.device)
        self.encoderIm = clip_model.encode_image(self.images)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        pass
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        ############################################################################
        txt_t = clip.tokenize( [query]).to(self.device)
        output_txt = self.model.encode_text(txt_t)
        sim=get_similarity_no_loop(output_txt,self.encoderIm)
        vals, indices = sim.topk(k, dim=1)
        top_indices = indices.view(-1).tolist()
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return top_indices
